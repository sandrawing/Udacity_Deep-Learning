...
length = 3
loader = batch_data(np.array(words), length, 2)
...
# 
# sequence_length = 10
# 我参考了一些论文后发现10是一个较为常用的参数，因此选择了10。
# 
# batch_size = 256
# batch_size如果太小的话，训练结果可能会很差；batch_size值增加，能提高训练速度，但是可能收敛结果变差。我尝试了128,256和512，发现256能使得模型的收敛速度最佳。
# 
# num_epochs = 25
# 我尝试了10,15,20和25，发现25能使得模型的收敛速度最佳。
# 
# learning_rate = 0.001 
# 学习率决定了模型中权重更新的速度，在迭代更新权重值的过程中，设置过大的值容易使训练的模型跨过最优值，导致过拟合；设置过小会使梯度下降过程过慢。尝试了0.01,0.001和0.0001后，发现0.001能使得模型的收敛速度最佳。
# 
# embedding_dim = 200
# 我尝试了100，200和300后，发现200能使得模型的收敛速度最佳。
# 
# hidden_dim = 300
# 根据课程中讲到的内容，hidden_dim参数最好选择200到500之间的值，在尝试了200,300,400和500后，发现300能使得模型的收敛速度最佳。
# 
# n_layers = 2
# 根据课程中讲到的内容，n_layers参数最好选择1-3之间的值，在尝试了2和3之后，发现2能使得模型的收敛速度最佳。
